 ...
 */
public final class NearestPillPacManVS2 extends AbstractPlayer
{	
	@Override
	public int getAction(Game game,long timeDue){		
		...
		int nearest=game.getTarget(current,targetsArray,true,G.DM.MANHATTEN);
...
//		GameView.addPoints(game,Color.GREEN,game.getPath(current,nearest));
		...
		if (game.getPowerPillIndicesActive() != null) {
			GameView.addPoints(game,Color.CYAN,game.getPath(current,game.getPowerPillIndicesActive()[0]));
		}
...
//				GameView.addPoints(game,colors[i],game.getGhostPath(i,current));
	...
	public String getGroupName() {
		return "Nearest Pill PacMan Visuals Modified";
	}
}